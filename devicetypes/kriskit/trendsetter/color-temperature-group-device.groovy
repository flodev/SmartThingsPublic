 ...
 */
metadata {
  definition (name: "Color Temperature Group Device", namespace: "kriskit.trendSetter", author: "Florian Biewald") {
    capability "Actuator"
    capability "Sensor"
    capability "Switch"
    ...
    attribute "colorTemperatureSync", "number"
  }
  ...
  tiles(scale: 2) {
    ...
    standardTile("colorTemperatureLabel", "colorTemperatureLable", height:1, width:1, decoration: "flat", inactiveLabel: true) {
      state "default", label:"CT", unit:"", icon: "st.Lighting.light11"
    }

    controlTile("colorTemperatureSliderControl", "device.colorTemperature", "slider", height: 1, width: 3, inactiveLabel: false, range:"(2200..6000)") {
      state "colorTemperature", action:"color temperature.setColorTemperature"
    }

    valueTile("colorTemperatureValue", "device.colorTemperature", inactiveLabel: true, height:1, width:1, decoration: "flat") {
      state "default", label:'${currentValue}'
    }

    valueTile("colorTemperatureSync", "device.colorTemperatureSync", height:1, width:1) {
      state "default", label:' Sync ', unit:"", action: "adjustColorTemperature", backgroundColor: "#ff9900"
      state "ok", label:'', unit:"", backgroundColor: "#00b509"
    }
  }
  ...
  main "switch"
  details([
    "switch", "levelLabel", "levelSliderControl", "levelValue", "levelSync",
    "colorTemperatureLabel", "colorTemperatureSliderControl", "colorTemperatureValue", "colorTemperatureSync"
  ])
}
...
def setColorTemperature(val, triggerGroup) {
  log.debug "Setting colorTemperature to $val"
  ...
  sendEvent(name: "temperature.setColorTemperature", value: val, isStateChange: true)
...
def syncColorTemperature(values) {
  log.debug "syncColorTemperature(): $values"
  ...
  if (matchValue == "bad") {
    log.debug "syncColorTemperature decided matchValue bad"
    colorTemperature = getAdjustmentColorTemperature(values)
  }

  log.debug "syncColorTemperature setColorTemperature $colorTemperature"
  setColorTemperature(colorTemperature, false)
  sendEvent(name: "colorTemperatureSync", value: matchValue, displayed: false)
}

def adjustColorTemperature() {
  ...
  def colorTemperature = getAdjustmentColorTemperature(values)

  log.debug "adjustColorTemperature values $values"
  log.debug "adjustColorTemperature setColorTemperature $colorTemperature"
  setColorTemperature(colorTemperature)
}

def getAdjustmentColorTemperature(values) {
  if (!values)
    return
  ...
  if (valueCountBy.size() > 1) {
    if (valueCountBy.size() == values.size()) {
      log.debug "Values are all different - making average"
      colorTemperature = Math.round(values.sum() / values.size())
    } else {
      log.debug "Some values are the same, choosing most popular"
      def firstItem = valueCountBy.find { true }
      colorTemperature = firstItem.key
    }
  }
